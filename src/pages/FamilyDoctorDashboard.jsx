...
import {
    ...
    CalendarDays, UserCircle, Pencil, GraduationCap, Briefcase, Building2, Globe, Phone, Mail, Save, Users2, Bell
} from 'lucide-react'
...
/* ================================================================
   PROFILE DATA
   ================================================================ */
const PROFILE_STORAGE_KEY = 'longevai-familydoctor-profile'

const DEFAULT_PROFILE = {
    name: 'Dr. Elena Gomez',
    title: 'Family Doctor -- Weekly Clinical Reviewer',
    license: 'MED-5614-FM',
    specialization: 'Family Medicine, Geriatric Primary Care & Chronic Disease Management',
    email: 'e.gomez@amatistalife.com',
    phone: '+34 611 567 890',
    office: 'Building B, Office 102',
    institution: 'Amatista Life -- LongevAI Center',
    education: 'MD, Family Medicine (Universidad Autonoma de Madrid)',
    certifications: 'Board Certified Family Physician, Geriatric Primary Care Certificate',
    bio: '12 years of experience in primary care for older adults. Focused on weekly clinical monitoring, medication management, and coordinating specialist care within the 16-week LongevAI cycle.',
    shiftStart: '08:30',
    shiftEnd: '16:30',
    yearsExperience: 12,
    residentsManaged: 4,
...
export default function FamilyDoctorDashboard() {
    ...
    const [notifiedSpecialists, setNotifiedSpecialists] = useState([])
    ...
    const handleReviewSubmit = () => {
        const specialists = []
        if (weeklyNotes.medications.trim()) specialists.push('Geriatrician', 'Nursing')
        if (weeklyNotes.nursing.trim()) specialists.push('Nursing Supervisor')
        if (weeklyNotes.referrals.trim()) specialists.push('Gerontologist')
        if (weeklyNotes.observations.trim()) specialists.push('Gerontologist')
        setNotifiedSpecialists([...new Set(specialists)])
        setReviewSubmitted(true)
        setTimeout(() => { setReviewSubmitted(false); setNotifiedSpecialists([]) }, 5000)
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <SectionCard title="Glucose & SpO2 Trend" icon={BarChart3} subtitle={selectedResident.name}>
                            <div className="h-56">
                                <ResponsiveContainer width="100%" height="100%">
                                    <LineChart data={data.vitals} margin={{ top: 5, right: 10, left: -10, bottom: 0 }}>
                                        ...
                                        <Line type="monotone" dataKey="temp" stroke="#C4545E" strokeWidth={2} name="Temp (C)" dot={{ r: 3 }} />
                                    </LineChart>
                                </ResponsiveContainer>
                            </div>
                            <div className="flex gap-3 mt-2 text-[10px] text-slate-500 flex-wrap">
                                ...
                                <span className="px-2 py-0.5 bg-emerald-50 border border-emerald-200 rounded text-emerald-700">Temp: 36-37.5C</span>
                            </div>
                        </SectionCard>
                    </div>
                </div>
                        ...
                        <div className="mb-4 space-y-2">
                            ...
                            {notifiedSpecialists.length > 0 && (
                                <div className="p-3 rounded-lg bg-blue-50 border border-blue-200 flex items-start gap-2">
                                    ...
                                    <div>
                                        <p className="text-sm font-medium text-blue-700">Auto-notifications sent</p>
                                        <p className="text-xs text-blue-600 mt-0.5">Notified: {notifiedSpecialists.join(', ')}</p>
                                    </div>
                                </div>
                            )}
                        </div>
